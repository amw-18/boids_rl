...
2.  **Kinematic Update:** We compute a theoretical "desired" velocity based on Newtonian momentum:
    $$ \vec{v}_{desired} = \vec{v}_t + F_{total} \Delta t $$
...
*   **Perceptual Threat Matrix:** We designed specific visual proxies for escaping predators:
    ...
    *   *Closing Speed ($v_{close}$):* The scalar rate at which the predator is approaching: $v_{close} = - (\vec{v}_p - \vec{v}_i) \cdot \vec{u}$. This is normalized to $[-1, 1]$ relative to the absolute maximum closure rate $v_{predator} + v_{base}$.
