...
def test_env_initialization():
    env = MurmurationEnv(num_agents=10, render_mode=None, device='cpu')
...
def test_env_step():
    ...
    assert len(obs) == 10
    assert obs['starling_0'].shape == env.observation_space('starling_0').shape
    ...
    assert not terminations['starling_0']
